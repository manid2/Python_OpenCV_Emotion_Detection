...
def recognize_emotion():
    ...
    print("The detected emotion is: %s" %
          emotions[max(set(predictions), key=predictions.count)])


while True:
    # Grab frame from webcam. Ret is 'true' if the frame was successfully
    # grabbed.
    ...
    for (x, y, w, h) in face:
        # draw it on the colour image "frame", with arguments: (coordinates),
        # (size), (RGB color), line thickness 2
        cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 0, 0), 2)
        cv2.putText(frame, 'Face detected!', (x, y),
                    font, 1, (0, 255, 0), 2, cv2.CV_AA)

    # Display frame
    cv2.imshow("Face Detection", frame)

    # imshow expects a termination definition in order to work correctly, here
    # it is bound to key 'q'
